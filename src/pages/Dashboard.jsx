    ...
    <div className="animate-slide-up space-y-6">
      ...
      <div className="bg-[#0f172a] rounded-2xl border border-[#1e293b] shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-[#1e293b] flex items-center justify-between">
          ...
          <button onClick={() => navigate('/orcamentos')} className="text-xs text-primary hover:underline font-medium">
            Ver todos
          </button>
        </div>
        {loading ? (
  ...
  const metrics = [
    {
      ...
      icon: Package, from: '#4c1d95', to: '#5b21b6', // violet-900 to violet-800
      iconBg: 'rgba(255,255,255,0.1)', route: '/estoque',
    },
    {
      name: 'Clientes', value: dados.totalClientes, suffix: 'cadastrados',
      icon: Users, from: '#5b21b6', to: '#6d28d9', // violet-800 to violet-700
      iconBg: 'rgba(255,255,255,0.15)', route: '/clientes',
    },
    {
      name: 'Orçamentos', value: dados.orcamentosMes, suffix: `em ${mesAtual}`,
      icon: FileText, from: '#6d28d9', to: '#7c3aed', // violet-700 to violet-600
      iconBg: 'rgba(255,255,255,0.2)', route: '/orcamentos',
    },
    {
      ...
      icon: TrendingUp, from: '#7c3aed', to: '#8b5cf6', // violet-600 to violet-500
      iconBg: 'rgba(255,255,255,0.25)', route: '/relatorios',
    },
  ];
  ...
  return (
    <div className="animate-slide-up space-y-5">
      ...
      <div className="flex justify-between items-center bg-slate-200 p-4 rounded-2xl border border-slate-300 shadow-sm">
        <div>
          <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">Resumo Estratégico</p>
          <p className="text-lg font-bold text-slate-900">Olá, Administrador</p>
        </div>
        <button onClick={fetchDados} className="text-xs text-slate-700 hover:text-primary transition-colors flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-slate-300/50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-[#0f172a] rounded-2xl border border-[#1e293b] overflow-hidden flex flex-col" style={{ boxShadow: '0 1px 12px rgba(0,0,0,0.04)' }}>
          <div className="px-5 py-4 border-b border-[#1e293b] flex items-center justify-between bg-amber-50/20">
            <div>
          ...
          <div className="flex-1 overflow-y-auto max-h-[350px]">
            {/* Alertas de Estoque */}
            <div className="px-5 py-3 bg-[#1e293b]/50/50">
            ...
            {dados.produtosBaixoEstoque.length === 0 ? (
              ...
              <div className="divide-y divide-[#1e293b]">
                {dados.produtosBaixoEstoque.map((p) => (
                  <div key={p.id} className="px-5 py-3 flex items-center justify-between hover:bg-amber-50/10 transition-colors">
            ...
            <div className="px-5 py-3 bg-[#1e293b]/50/50 border-t border-[#1e293b]">
              <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest flex items-center gap-2">
            ...
            {dados.orcamentosPendentes.length === 0 ? (
              ...
              <div className="divide-y divide-[#1e293b]">
                {dados.orcamentosPendentes.map((orc) => (
                  <div key={orc.id} className="px-5 py-3 flex items-center justify-between hover:bg-amber-50/10 transition-colors cursor-pointer" onClick={() => navigate('/orcamentos')}>
          ...
          <div className="p-3 bg-[#1e293b]/30 border-t border-[#1e293b] text-center">
            <button onClick={fetchDados} className="text-[10px] text-primary font-bold uppercase tracking-wider flex items-center justify-center gap-1 mx-auto hover:underline">
